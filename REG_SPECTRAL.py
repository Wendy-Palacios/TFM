...
# Configurar y aplicar Spectral
spect = cluster.SpectralClustering(
    n_clusters=4,
    eigen_solver='arpack',
    gamma=1.0,
    affinity='rbf',
...
SOC_cluster_4 = SOC[SOC['cluster'] == 3]
...
X4 = SOC_cluster_4.iloc[:, 0:2]
y4 = SOC_cluster_4.iloc[:, -2].values
...
def train_and_evaluate(X_train, y_train, X_test, y_test, name_model, ax, i):
    ...
    print("Error cuadrático medio (MASE):", mase)
    print("Error cuadrático medio (SMAPE):", smape)
    print("Error cuadrático medio (LMLS):", lmls)
...
for modelo in name_model:
    ...
    for i, (X, y) in enumerate([(X1, y1), (X2, y2), (X3, y3), (X4, y4)], start=1):
        ...
        results_df = results_df._append({
            'Modelo': modelo,
            'Nº Cluster': i,
            'Best Params': best_model,  # No need to convert to string here
            'MSE': mse,
            'MAE': mae,
            'MASE': mase,
            'SMAPE' : smape,
