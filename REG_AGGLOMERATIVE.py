...
def train_and_evaluate(X_train, y_train, X_test, y_test, name_model, ax, i):
    ...
    if name_model == 'MLP':
        model = MLPRegressor(max_iter=10000)  # Agregar max_iter para evitar convergencia prematura
        param_grid = {
    ...
    lmls = np.mean(np.log(((((y_pred - y_test) ** 2)) + 1) / 2))#np.mean(np.log(((np.square(y_pred - y_test)) + 1) / 2))#mean_squared_log_error(y_test, y_pred)
